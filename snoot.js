...
function updateDays() {
	...
	while (dates[28]) {
		// remove child with index 28 until this index is empty
...
/* remove default value and formatting from state and delivery date selection lists*/
...
function clearPlaceholder() {
	var messageBox = getElementById("customText");
...
/* for IE8 and early displayed text in placeholder in the textarea box*/
function generatePlaceholder() {
	if (!Modernizr.input.placeholder) {
		...
		if (messageBox.addEventListener) {
			messageBox.addEventListener("focus", clearPlaceholder, false);
			messageBox.addEventListener("blur", checkPlaceholder, false);
		} else if (messageBox.attachEvent) {
			messageBox.attachEvent("onfocus", clearPlaceholder);
			messageBox.attachEvent("onblur", checkPlaceholder);
		}
	}
}

/* automatically check Custom message check box if user makes entry customText box*/
...
function validatePayment() {
	...
	var cards = document.getElementsByClassName("PaymentType");
	var currentElement;
	try {
		if (!cards[0].checked && !cards[1].checked && !cards[2].checked && !cards[3].checked) {
			// verify that card selected
			for (var i = 0; i < 4; i++) {
		...
		for (var i = 0; i < elementCount; i++) {
			// verify that a mounth and year have been selected
			currentElement = selectElements[i];
			if (currentElement.selectedIndex === -1) {
				currentElement.style.border = "1px solid red";
				fieldsetValidity = false;
...
function validateForm(evt) {
	if (evt.preventDefault) {
		...
		evt.returnValue = false; // prevent form submitting in IE8 
	}
